...
require dirname(__FILE__).'/Driver/Mysql.class.php';
require dirname(__FILE__).'/Driver/Lite.class.php';
class Model{
    // 当前数据库操作对象
    protected $db               =   null;
    ...
    {
        $md5    =   md5(serialize($param));
        if(!isset(self::$instance[$md5])) {
            // 解析连接参数 支持数组和字符串
            $options    =   self::parseConfig($param);
            $this->tablePrefix = $param['DB_PREFIX'];
            // 兼容mysqli
            if('mysqli' == $options['type']) $options['type']   =   'mysql';
            // 如果采用lite方式 仅支持原生SQL 包括query和execute方法
            $class  =   $options['lite']?  'DB\\Driver\\Lite' :   'DB\\Driver\\'.ucwords(strtolower($options['type']));//把每个单词的首字符转换为大写
            if(class_exists($class)){
                self::$instance[$md5]   =   new $class($options);
            }else{
                // 类没有定义
                $this->err($options['type'].'\'driver is not exsit!');
            }
        }
        self::$_instance    =   self::$instance[$md5];

        $this->db = self::$instance[$md5];
        return $this;
    }
    ...
    public function __call($method,$args) {
        if(in_array(strtolower($method),$this->methods,true)) {
            ...
            $this->err(__CLASS__.':'.$method.'_METHOD_NOT_EXIST_');
            return;
        }
    }
    // 回调方法 初始化模型
            ...
            foreach ($data as $key=>$val){
                if(!in_array($key,$fields,true)){
                    if(!empty($this->options['strict'])){
                        $this->err('_DATA_TYPE_INVALID_'.':['.$key.'=>'.$val.']');
                    }
                    unset($data[$key]);
        ...
        if(isset($options['where']) && is_array($options['where']) && !empty($fields) && !isset($options['join'])) {
            ...
            foreach ($options['where'] as $key=>$val){
                ...
                if(in_array($key,$fields,true)){
                    ...
                    if(!empty($this->options['strict'])){
                        $this->err('_ERROR_QUERY_EXPRESS_'.':['.$key.'=>'.$val.']');
                    }
