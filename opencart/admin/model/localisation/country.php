...
class ModelLocalisationCountry extends Model {
	...
	public function getCountries($data = array()) {
		if ($data) {
			...
			if (!$country_data) {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "country  where country_id = 44 ORDER BY name ASC");
	...
	public function getTotalCountries() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "country  where country_id = 44");
